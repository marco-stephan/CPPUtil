...
#include <tuple>
#include "Util/Conversion/Integer.h"
...
namespace Util_Tests
{
	namespace Conversion_Tests
	{
		...
		{
			ASSERT_TRUE(Util::Conversion::Integer::ToBool(1));
			ASSERT_TRUE(Util::Conversion::Integer::ToBool(7));
			ASSERT_TRUE(Util::Conversion::Integer::ToBool(-7));

			ASSERT_FALSE(Util::Conversion::Integer::ToBool(0));
		}
		...
		TEST_F(Conversion_Integer_Test, ToCStringInt)
		{
			for (size_t i = 0; i < STRING_INT_PAIRS.size(); i++)
			{
				...
				const char* string = Util::Conversion::Integer::ToCString(intValue);
		...
		TEST_F(Conversion_Integer_Test, ToCStringUnsignedInt)
		{
			for (size_t i = 0; i < STRING_UNSIGNED_INT_PAIRS.size(); i++)
			{
				...
				const char* string = Util::Conversion::Integer::ToCString(unsignedIntValue);
